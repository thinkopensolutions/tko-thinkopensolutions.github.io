...
$LOCAL_REPO_NAME    = "thinkopensolutions.github.io";
...
if ( $_POST['payload'] ) {
  ...
  if( file_exists($LOCAL_REPO) ) {
    ...
    shell_exec("cd {$LOCAL_REPO} && git pull && jekyll build");
    die("done " . mktime());
  } else {
    
    // If the repo does not exist, then clone it into the parent directory
    shell_exec("cd {$LOCAL_ROOT} && git clone {$REMOTE_REPO} && cd {$LOCAL_REPO} && jekyll build");
